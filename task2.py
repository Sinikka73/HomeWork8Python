...
все оставшиеся. Например, количество ячеек клетки равняется 12,
...
class Cell:
    ...
    def __sub__(self, other):
        ...
        if self.quantity - other.quantity < 0:
            return 'Действие не выполняется, потому что разность отрицательна'
    ...
    def make_order(self, row):
        ...
        :return: организацию ячеек по рядам
        """
        order = ['*' * row for i in range(self.quantity // row)]
        order.append('*' * (self.quantity % row))
        return '\n'.join(order)
